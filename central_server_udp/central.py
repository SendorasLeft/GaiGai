...
def update_radio_membership(radio_idx, channel):
    ...
    if channel_prefs[radio_idx] == channel:
        ...
        if channel != -1 and channel_prefs[radio_idx] == -1:
            ...
            print("[CHANNEL CHANGE] Update: Radio", radio_idx, "has switched from channel", channel_prefs[radio_idx], "to", channel)
    ...
    while not_shutdown:
        ...
        for client in subscribed_sockets:
            try:
                ...
                print("[WARNING] PREF Socket Timed out") # sanity check, it shouldn't ever reach here due to select-checking
                if channel_prefs[radio_idx] != -1: 
                    update_radio_membership(radio_idx, -1)
...
def main():
    ...
    channelprefs_sockets = subscription_multicast_setup(multicast_ip=MULTICAST_IP, subscription_ports=CHANNEL_PREF_PORTS)
